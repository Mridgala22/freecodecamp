...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = round(
        (df[higher_education]['salary'] == '>50K').sum() / len(df[higher_education]) * 100, 1
    )
    lower_education_rich = round(
        (df[lower_education]['salary'] == '>50K').sum() / len(df[lower_education]) * 100, 1
    )
    ...
    # Calculate percentage of >50K earners per country
    country_total = df['native-country'].value_counts()
    country_rich = df[df['salary'] == '>50K']['native-country'].value_counts()
    
    # Divide matching countries and fill missing ones with 0
