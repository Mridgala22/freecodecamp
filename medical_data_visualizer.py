...
# BMI = weight (kg) / [height (m)]^2. Height in dataset is in cm, so divide by 100.
...
def draw_cat_plot():
    ...
    # Rename the column to 'total' so the catplot renders correctly
    df_cat = df_cat.groupby(['cardio', 'variable', 'value']).size().reset_index(name='total')
    

    # 7. Convert the data into long format and create the chart using sns.catplot()
